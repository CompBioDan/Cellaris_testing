...
void GetParticleBounds(Vec3& lower, Vec3& upper, SimBuffers* flex_buffers)
{
	lower = Vec3(FLT_MAX);
	upper = Vec3(-FLT_MAX);
...
{
	if (rigid && flex_buffers->rigidIndices.empty())
		flex_buffers->rigidOffsets.push_back(0);
	...
	for (int x = 0; x < dimx; ++x)
	{
		for (int y = 0; y < dimy; ++y)
		{
			...
			{
				if (rigid)
					flex_buffers->rigidIndices.push_back(int(flex_buffers->positions.size()));

				//Vec3 position = lower + Vec3(float(x), float(y), float(z))*radius + RandomUnitVector()*jitter;
				Vec3 position = lower + Vec3(float(x), float(y), float(z))*(radius*4);

				flex_buffers->positions.push_back(Vec4(position.x, position.y, position.z, invMass));
				flex_buffers->velocities.push_back(velocity);
	...
	if (rigid)
	{
		flex_buffers->rigidCoefficients.push_back(rigidStiffness);
		flex_buffers->rigidOffsets.push_back(int(flex_buffers->rigidIndices.size()));
	}
}
...
void CreateSpring(SimBuffers* flex_buffers,int i, int j, float stiffness, float give=0.0f)
{
	flex_buffers->springIndices.push_back(i);
	flex_buffers->springIndices.push_back(j);
	flex_buffers->springLengths.push_back((1.0f+give)*Length(Vec3(flex_buffers->positions[i])-Vec3(flex_buffers->positions[j])));
...
void CreateParticleShape(const Mesh* srcMesh, Vec3 lower, Vec3 scale, float rotation, float spacing, Vec3 velocity, float invMass, bool rigid, float rigidStiffness, int phase, bool skin, float jitter=0.005f, Vec3 skinOffset=0.0f, float skinExpand=0.0f, Vec4 color=Vec4(0.0f), float springStiffness=0.0f)
{
	...
	int startIndex = int(flex_buffers->positions.size());
			...
			for (int y=0; y < maxDim; ++y)
			{
				for (int z=0; z < maxDim; ++z)
				{
					...
					if (voxels[index])
					{
						...
						if (rigid)
							flex_buffers->rigidLocalNormals.push_back(Vec4(n, d));

						// track which particles are in which cells
						indices[index] = flex_buffers->positions.size();
	...
	if (skin)
	{
		flex_buffers->rigidMeshSize.push_back(mesh.GetNumVertices());
		...
		for (uint32_t i=startVertex; i < g_mesh->GetNumVertices(); ++i)
		{
			...
			for (int j=startIndex; j < flex_buffers->positions.size(); ++j)
			{
				float dSq = LengthSq(Vec3(g_mesh->m_positions[i])-Vec3(flex_buffers->positions[j]));
...
void SkinMesh()
{
	if (g_mesh)
	{
		...
		for (int r=0; r < flex_buffers->rigidRotations.size(); ++r)
		{
			const Matrix33 rotation = flex_buffers->rigidRotations[r];
			const int numVertices = flex_buffers->rigidMeshSize[r];
			...
			for (int i=startVertex; i < numVertices+startVertex; ++i)
			{
				...
				for (int w=0; w < 4; ++w)
				{
					...
					if (g_meshSkinIndices[i*4+w] > -1)
					{
						...
						skinPos += (rotation*(g_meshRestPositions[i]-Point3(flex_buffers->restPositions[index])) + Vec3(flex_buffers->positions[index]))*weight;
					}
				}
...
{
	// transform
	flex_buffers->shapePositions.push_back(Vec4(center.x, center.y, center.z, 0.0f));
	flex_buffers->shapeRotations.push_back(quat);

	flex_buffers->shapePrevPositions.push_back(flex_buffers->shapePositions.back());
	flex_buffers->shapePrevRotations.push_back(flex_buffers->shapeRotations.back());
	...
	flex_buffers->shapeGeometry.push_back(geo);
	flex_buffers->shapeFlags.push_back(NvFlexMakeShapeFlags(eNvFlexShapeBox, dynamic));
}
...
void AddSphere(float radius, Vec3 position, Quat rotation)
{
	...
	flex_buffers->shapePositions.push_back(Vec4(position, 0.0f));
	flex_buffers->shapeRotations.push_back(rotation);

	flex_buffers->shapePrevPositions.push_back(flex_buffers->shapePositions.back());
	flex_buffers->shapePrevRotations.push_back(flex_buffers->shapeRotations.back());

	int flags = NvFlexMakeShapeFlags(eNvFlexShapeSphere, false);
	flex_buffers->shapeFlags.push_back(flags);
}
...
void AddRandomConvex(int numPlanes, Vec3 position, float minDist, float maxDist, Vec3 axis, float angle)
{
	...
	flex_buffers->shapePositions.push_back(Vec4(position.x, position.y, position.z, 0.0f));
	flex_buffers->shapeRotations.push_back(QuatFromAxisAngle(axis, angle));
...
void CreateRandomBody(int numPlanes, Vec3 position, float minDist, float maxDist, Vec3 axis, float angle, float invMass, int phase, float stiffness)
{
	...
	for (size_t v=0; v < builder.mVertices.size(); ++v)
	{
		...
		flex_buffers->positions.push_back(Vec4(p.x, p.y, p.z, invMass));
		...
		for (size_t i=v+1; i < builder.mVertices.size(); ++i)
		{
			...
			flex_buffers->springIndices.push_back(a);
			flex_buffers->springIndices.push_back(b);
			flex_buffers->springLengths.push_back(Length(builder.mVertices[v]-builder.mVertices[i]));
			flex_buffers->springStiffness.push_back(stiffness);
	...
	for (size_t t=0; t < builder.mIndices.size(); ++t)
		flex_buffers->triangles.push_back(startIndex + builder.mIndices[t]);		

	// lazy
	flex_buffers->triangleNormals.resize(flex_buffers->triangleNormals.size() + builder.mIndices.size()/3, Vec3(0.0f));
}
...
void AddTriangleMesh(NvFlexTriangleMeshId mesh, Vec3 translation, Quat rotation, Vec3 scale)
{
	...
	flex_buffers->shapePositions.push_back(Vec4(translation, 0.0f));
	flex_buffers->shapeRotations.push_back(Quat(rotation));
	flex_buffers->shapePrevPositions.push_back(Vec4(translation, 0.0f));
	flex_buffers->shapePrevRotations.push_back(Quat(rotation));
	flex_buffers->shapeGeometry.push_back((NvFlexCollisionGeometry&)geo);
	flex_buffers->shapeFlags.push_back(NvFlexMakeShapeFlags(eNvFlexShapeTriangleMesh, false));
}
...
void AddSDF(NvFlexDistanceFieldId sdf, Vec3 translation, Quat rotation, float width)
{
	...
	flex_buffers->shapePrevRotations.push_back(Quat(rotation));
	flex_buffers->shapeGeometry.push_back((NvFlexCollisionGeometry&)geo);
	flex_buffers->shapeFlags.push_back(NvFlexMakeShapeFlags(eNvFlexShapeSDF, false));
}
...
{
	int baseIndex = int(flex_buffers->positions.size());
	...
	for (int z=0; z < dz; ++z)
	{
		for (int y=0; y < dy; ++y)
		{
			for (int x=0; x < dx; ++x)
			{
				...
				if (x > 0 && y > 0)
				{
					flex_buffers->triangles.push_back(baseIndex + GridIndex(x-1, y-1, dx));
					flex_buffers->triangles.push_back(baseIndex + GridIndex(x, y-1, dx));
					flex_buffers->triangles.push_back(baseIndex + GridIndex(x, y, dx));
					
					flex_buffers->triangles.push_back(baseIndex + GridIndex(x-1, y-1, dx));
					flex_buffers->triangles.push_back(baseIndex + GridIndex(x, y, dx));
					flex_buffers->triangles.push_back(baseIndex + GridIndex(x-1, y, dx));

					flex_buffers->triangleNormals.push_back(Vec3(0.0f, 1.0f, 0.0f));
					flex_buffers->triangleNormals.push_back(Vec3(0.0f, 1.0f, 0.0f));
				}
			}
		}
...
{
	rope.mIndices.push_back(int(flex_buffers->positions.size()));

	flex_buffers->positions.push_back(Vec4(start.x, start.y, start.z, invmass));
	flex_buffers->velocities.push_back(0.0f);
	flex_buffers->phases.push_back(phase);//int(flex_buffers->positions.size()));
...
void CreateTetMesh(const char* filename, Vec3 lower, float scale, float stiffness, int phase)
{
	...
	if (f)
	{
		...
		const int vertOffset = flex_buffers->positions.size();
				...
				case 'v':
					{
						...
						flex_buffers->positions.push_back(Vec4(pos.x, pos.y, pos.z, 1.0f));
						flex_buffers->velocities.push_back(0.0f);
						flex_buffers->phases.push_back(phase);
				...
				case 't':
					{
						if (firstTet)
						{
							...
							// normalize positions
							for (int i=vertOffset; i < int(flex_buffers->positions.size()); ++i)
							{
								Vec3 p = lower + (Vec3(flex_buffers->positions[i])-meshLower)*scale/maxEdge;
								flex_buffers->positions[i] = Vec4(p, flex_buffers->positions[i].w);
							}
		...
		for (TriMap::iterator iter=triCount.begin(); iter != triCount.end(); ++iter)
		{
			...
			if (iter->second == 1)
			{
				flex_buffers->triangles.push_back(key.orig[0]);
				flex_buffers->triangles.push_back(key.orig[1]);
				flex_buffers->triangles.push_back(key.orig[2]);
				flex_buffers->triangleNormals.push_back(0.0f);
			}
		}
...
void GetShapeBounds(Vec3& totalLower, Vec3& totalUpper)
{
	...
	for (int i=0; i < flex_buffers->shapeFlags.size(); ++i)
	{
		NvFlexCollisionGeometry geo = flex_buffers->shapeGeometry[i];

		int type = flex_buffers->shapeFlags[i]&eNvFlexShapeFlagTypeMask;
		...
		TransformBounds(localLower, localUpper, Vec3(flex_buffers->shapePositions[i]), flex_buffers->shapeRotations[i], 1.0f, worldLower, worldUpper);
