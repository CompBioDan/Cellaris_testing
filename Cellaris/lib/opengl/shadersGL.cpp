...
FluidRenderBuffers CreateFluidRenderBuffers(int numFluidParticles, bool enableInterop)
{
	...
	if (enableInterop)
	{
		...
		buffers.mPositionBuf = NvFlexRegisterOGLBuffer(flex_library, buffers.mPositionVBO, numFluidParticles, sizeof(Vec4));
		buffers.mDensitiesBuf = NvFlexRegisterOGLBuffer(flex_library, buffers.mDensityVBO, numFluidParticles, sizeof(float));
		buffers.mIndicesBuf = NvFlexRegisterOGLBuffer(flex_library, buffers.mIndices, numFluidParticles, sizeof(int));

		buffers.mAnisotropyBuf[0] = NvFlexRegisterOGLBuffer(flex_library, buffers.mAnisotropyVBO[0], numFluidParticles, sizeof(Vec4));
		buffers.mAnisotropyBuf[1] = NvFlexRegisterOGLBuffer(flex_library, buffers.mAnisotropyVBO[1], numFluidParticles, sizeof(Vec4));
		buffers.mAnisotropyBuf[2] = NvFlexRegisterOGLBuffer(flex_library, buffers.mAnisotropyVBO[2], numFluidParticles, sizeof(Vec4));
	}
...
DiffuseRenderBuffers CreateDiffuseRenderBuffers(int numDiffuseParticles, bool& enableInterop)
{
	...
	if (numDiffuseParticles > 0)
	{
		...
		if (enableInterop)
		{
			...
			buffers.mDiffuseIndicesBuf = NvFlexRegisterOGLBuffer(flex_library, buffers.mDiffuseIndicesIBO, numDiffuseParticles, sizeof(int));
			buffers.mDiffusePositionsBuf = NvFlexRegisterOGLBuffer(flex_library, buffers.mDiffusePositionVBO, numDiffuseParticles, sizeof(Vec4));
			buffers.mDiffuseVelocitiesBuf = NvFlexRegisterOGLBuffer(flex_library, buffers.mDiffuseVelocityVBO, numDiffuseParticles, sizeof(Vec4));
		}
	}
